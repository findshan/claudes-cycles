...
def print_cycle(m, cycle, path):
    ...
    for idx, ((i, j, k), d) in enumerate(path):
        if idx < 5 or idx >= len(path) - 3:
            print(f"    ({i},{j},{k}) -> {DIR_NAMES[d]}")
        elif idx == 5:
            print(f"    ... ({len(path) - 8} more steps) ...")


def main():
    import time
    test_ms = [4, 8, 16, 24, 6,10,18]
    print("=== Claude's Cycles — 偶数哈密顿回路分解 (Knuth's construction) ===\n")
    for m in test_ms:
        print(f">>> m = {m}  (顶点数: {m**3:,},  弧数: {3*m**3:,})")
        t0 = time.time()
        fibers = build_even_decomposition(m)
        verify_decomposition(m, fibers)
        elapsed = time.time() - t0
        print(f"    [✓] 3 条哈密顿回路构造并验证通过  耗时: {elapsed:.3f}s")
        # print first cycle briefly
        path = generate_cycle(m, 0, fibers)
        print_cycle(m, 0, path)
        print()
